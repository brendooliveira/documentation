...
<CardGroup cols={3}>
  ...
  <Card title="Referência da API" icon="terminal" href="/api-reference/introducao">
    As nove operações, com playground.
  </Card>
...
**Nove operações** de servidor, todas sob `/v1` e autenticadas por chave secreta:
...
- **O número do cartão não passa por você.** `POST /v1/charges` cria cobrança no cartão, mas o que
  ela recebe é um `token` de uso único, gerado no navegador do comprador pela biblioteca da
  Pagar.me — nunca o PAN. Omitindo `payment_method`, a cobrança continua sendo PIX, como sempre foi.
  Ver [Cobrança no cartão](/essenciais/cartao).
