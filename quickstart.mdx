...
`items` é obrigatório: discrimine o que está sendo vendido. Vendendo uma coisa só, mande um item
único com o valor cheio — a soma de `quantity * amount` de todos os itens precisa bater exatamente
com `amount`.
...
$resposta = Http::withToken(config('services.vext.key'))
    ...
    ->post('https://api.usevext.com/v1/charges', [
        ...
        'items' => [                         // obrigatório: a soma tem que bater com amount
            [
                'code' => 'curso-vue',
                'description' => 'Curso de Vue 3',
                'quantity' => 1,
...
const resposta = await fetch('https://api.usevext.com/v1/charges', {
  ...
  body: JSON.stringify({
    ...
    items: [
      // obrigatório: a soma tem que bater com amount
      {
        code: 'curso-vue',
        description: 'Curso de Vue 3',
        quantity: 1,
...
```json
{
  ...
  "pix": {
    ...
    "expires_at": "2026-08-05T14:00:00+00:00",
    "end_to_end_id": null
  },
...
<Note>
  A cobrança nasce `pending` e vale 24 horas por padrão. Ajuste com `expires_in`, em segundos, entre 60
  e 604.800.
</Note>
...
<CardGroup cols={2}>
  ...
  <Card title="Webhooks" icon="webhook" href="/webhooks/visao-geral">
    Os nove eventos, assinatura e retentativas.
  </Card>
