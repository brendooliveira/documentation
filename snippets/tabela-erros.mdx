...
| `one_click_requires_three_ds` | 403 | `permission_error` | A loja exige [autenticação do portador](/essenciais/autenticacao-do-portador), que não convive com cartão guardado |
| `resource_not_found` | 404 | `not_found_error` | O recurso não existe — ou é de outra conta |
| `card_not_found` | 404 | `not_found_error` | O `pm_` não existe, ou é de outro comprador |
| `card_declined` | 402 | `card_error` | A operadora recusou. **A cobrança existe**, em `details.charge_id`, com status `failed` |
| `pix_failed` | 402 | `api_error` | Não conseguimos gerar o código Pix. Ofereça o cartão, ou tente de novo |
...
| `card_expired` | 422 | `invalid_request_error` | O cartão guardado venceu. Peça um novo ao comprador |
| `card_revoked` | 422 | `invalid_request_error` | O cartão guardado foi removido, por você ou pelo comprador |
| `card_not_usable_on_account` | 422 | `invalid_request_error` | O cartão não vale na conta que está cobrando. Ver [Cofre de cartões](/essenciais/cofre-de-cartoes) |
