...
curl "https://api.vext.com.br/v1/charges?reference=pedido-1042" \
...
curl -G "https://api.vext.com.br/v1/charges" \
  -H "Authorization: Bearer sk_live_..." \
...
do {
    $resposta = Http::withToken(config('services.vext.key'))
        ->get('https://api.vext.com.br/v1/charges', [
            'status' => 'paid',
            'created_from' => $inicio,
            'created_to' => $fim,
            'per_page' => 100,
...
while (temMais) {
  const url = new URL('https://api.vext.com.br/v1/charges');
