...
description: "A aritmética de devolver R$ 40,00 de uma venda de R$ 100,00 e o que o vendedor vê depois."
---
...
$resposta = Http::withToken(config('services.vext.key'))
    ...
    ->post("https://api.vext.com.br/v1/charges/{$pedido->cobranca_id}/refund", [
...
const resposta = await fetch(
  `https://api.vext.com.br/v1/charges/${pedido.cobrancaId}/refund`,
  {
    method: 'POST',
    headers: {
...
resposta = requests.post(
    f"https://api.vext.com.br/v1/charges/{pedido.cobranca_id}/refund",
    headers={
...
<Warning>
  Envie `amount` sempre que o estorno for parcial. Omitir devolve **todo o saldo estornável** da
  cobrança — numa venda de R\$ 100,00 ainda intacta, isso são R\$ 100,00, e não os R\$ 40,00 que você
  queria.
</Warning>
