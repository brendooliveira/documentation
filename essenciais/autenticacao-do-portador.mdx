...
Você **pede**; a decisão é nossa. O campo é `three_d_secure`, em `POST /v1/charges` e em
`POST /v1/checkout_sessions`, com três valores:
...
<Step title="O valor chegou ao piso da plataforma?">
  Se chegou, autentica. Esse piso vale para a API pública e para o embed, onde existe um valor para
  comparar.
</Step>
...
## No embed, você não faz nada

O quadro cuida do desafio inteiro — pede o token, carrega a biblioteca do autenticador, expande-se
sobre a sua página quando o banco pergunta a senha e volta ao tamanho normal ao terminar. No
[Drop-in](/embed/drop-in) não há sequer o que configurar.

No [Elements](/embed/elements) a única obrigação é mandar os dados: `amountCents`, `customer` e
`billingAddress` no `createToken`. Sem eles a autenticação não tem como acontecer, e a chamada falha
com `three_ds_data_missing` nomeando o que faltou.

Depois, repasse ao seu servidor o objeto `threeDs` **exatamente como veio**, e envie-o em
`card.three_ds`. Esse par `transaction_id` + `status` é a prova da autenticação: perdê-lo no caminho
significa ter incomodado o comprador com um desafio e pagado a contestação assim mesmo.
...
<Warning>
  ...
  cartão em claro, e um cartão no cofre é uma referência sem número — por isso uma loja que exige
  autenticação não oferece [Vext Link](/embed/vext-link), e um `card_id` nela volta `403`
  `one_click_requires_three_ds`. Cobrar assim mesmo, fingindo que autenticou, entregaria a venda sem
  transferência de responsabilidade, e a loja descobriria isso depois de já ter perdido a
  contestação.
</Warning>
...
| `code` | HTTP | O sintoma |
| --- | --- | --- |
| `three_ds_data_missing` | — | No navegador. Falta `amountCents`, `customer` ou `billingAddress` |
| `three_ds_failed` | — | No navegador. O desafio não completou |
...
O token que inicia a autenticação vale **cinco minutos** e serve uma vez só. O quadro o pede no
momento em que o comprador envia o formulário, e não quando a página abre: emitido cedo, ele já
estaria morto para quem levou algum tempo preenchendo o endereço.
