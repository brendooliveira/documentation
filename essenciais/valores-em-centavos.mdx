...
```json Cobrança de 10000 centavos (R\$ 100,00)
{
  ...
  "items": [
    { "description": "Curso de Vue 3", "quantity": 1, "amount": 10000 }
  ]
}
```
...
<Note>
  `items` é obrigatório em `POST /v1/charges`, e não dispensa `amount`. A soma de `quantity * amount`
  de todos os itens precisa bater exatamente com o `amount` da cobrança — sem essa conferência, o
  comprador veria um total diferente da soma do que está levando. Vendendo uma coisa só, mande um
  item único com o valor cheio.
</Note>
