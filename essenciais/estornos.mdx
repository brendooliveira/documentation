---
title: "Estornos"
description: "Quem paga o quê quando um estorno acontece, e por que estorno nunca é barrado por falta de saldo."
icon: "rotate-ccw"
---
...
```bash Estorno parcial de 4000 centavos
...
```json Estorno integral de uma venda de 10000 centavos
{
  "object": "refund",
...
| `code` | Por quê |
| --- | --- |
| `charge_not_refundable` | A cobrança não está paga, ou já foi levada por um chargeback. Uma venda aguardando pagamento não tem o que devolver; uma `chargedback` já devolveu |
...
## Chargeback também vira um `refund`

Quando o portador contesta a compra na operadora, a bandeira leva o valor de volta e nós registramos
o débito num objeto `refund` — o mesmo formato de um estorno que você pediu. Assim o seu código lê
os dois pelo mesmo caminho, em vez de manter duas leituras para o mesmo fato contábil.

A diferença está no que você pode fazer depois: nenhuma. A cobrança fica `chargedback`, e pedir
estorno sobre ela devolve `422 charge_not_refundable`. O aviso chega como
[`charge.chargedback`](/api-reference/webhooks/charge-chargedback), e é nele que você revoga o que
já entregou.
