---
title: "Erros"
description: "Um envelope único, oito famílias de tipo e o catálogo estável de códigos."
icon: "triangle-alert"
---
...
| `card_error` | O cartão do comprador | Mostre a `message` e ofereça outro meio. Não repita |
...
`card_error` é o único que descreve uma **cobrança que existe**. Os outros sete recusam a chamada
antes de criar qualquer coisa; um `402 card_declined` devolve em `details.charge_id` o `ch_` de uma
cobrança já gravada com status `failed`. Tratá-lo como os demais e descartar a resposta deixa você
sem a tentativa que o comprador jura ter feito. Ver
[Cobrança no cartão](/essenciais/cartao#ao-contrário-do-pix-uma-recusa-cria-cobrança).
...
Os erros que só aparecem no navegador — chave publicável ausente, revogada ou fora dos domínios
autorizados — estão em [Chave publicável](/embed/chave-publicavel#os-erros-que-você-vai-ver). Eles
nunca chegam a uma chamada feita com `sk_`.
